...
function parse(dir) {
    fs.readdir(dir, function (err, filenames) {
        filenames.forEach(function (filename) {
            fs.readFile(path.resolve(dir, filename), 'utf-8', function (err, result) {
                if (err) {
                    console.error(err);
                    process.exit(1);
                }
                return parseFile(filename.slice(0, -4), result);
            });
        });
    });
}

function parseFile(fileName, fileData) {
    let stringDatos = fileData.split("\"\n");
...
function parseMateria(texto) {
    var str = texto.replace(/\t/g, " ").replace(/" {2}"/g, " ");
    var pos = str.indexOf(" ", str.indexOf("Materia:"));
    var codigo = str.slice(pos + 1, pos + 5);
    var pos2 = str.indexOf("Vacantes", pos);
    var nombre = str.slice(pos + 6, pos2 - 1);

    if (Number(codigo) < 5000 || Number(codigo) > 9999) {
...
function parseCarreras(texto) {
    ...
    if (carreras.length === 1 && carreras[0] === "") {
        return carrerasFlags;
    }

    if (carreras.length === 1 && carreras[0] === "Todas") {
        for (let carreraConst of ConstDatosCarreras) {
            carrerasFlags.push(carreraConst.flag);
        }
        return carrerasFlags;
    }
    ...
    for (let carrera of carreras) {
        let ok = false;
        for (let carreraConst of ConstDatosCarreras) {
            if (carrera === carreraConst.nombre) {
                carrerasFlags.push(carreraConst.flag);
                ok = true;
            }
        }
        ...
        if (!ok) {
            console.error("Carrera inválida: \"" + carrera + "\"");
            process.exit(1);
        }
    }
...
var AllCarrerasFlags = Object.keys(CarrerasFlags);
AllCarrerasFlags.shift();
//console.log(AllCarrerasFlags);
